...
| settings.pricingEndpointRegion | string | `""` | AWS region containing the Price List Query API endpoint. If not set, Karpenter selects an endpoint region based on the cluster region. This does not change the region for which prices are retrieved and has no effect when isolatedVPC is enabled. |
