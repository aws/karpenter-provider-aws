...
| settings.awsFeatureGates | object | `{"dra":false,"nodeClassCEL":false}` | AWS-specific Feature Gate configuration values. |
| settings.awsFeatureGates.dra | bool | `false` | dra is ALPHA and is disabled by default. Setting this to true will make Karpenter calculate the DRA devices the NVIDIA GPU and EFA drivers publish, so it can account for DRA claims when it sizes new nodes. |
