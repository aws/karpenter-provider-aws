...
```shell
cosign verify public.ecr.aws/karpenter/karpenter:1.6.0 \
...
| controller.image.tag | string | `"1.6.0"` | Tag of the controller image. |
...
| settings.pricingRegionOverride | string | `""` | Override the AWS region for Pricing API requests. If not specified, defaults to: eu-central-1 (eu-* clusters), ap-south-1 (ap-* clusters), cn-northwest-1 (cn-* clusters), us-east-1 (all others) |
