...
| settings | object | `{"amiRefreshInterval":"1m","awsFeatureGates":{"nodeClassCEL":false},"batchIdleDuration":"1s","batchMaxDuration":"10s","clusterCABundle":"","clusterDNSIP":"","clusterEndpoint":"","clusterName":"","disableClusterStateObservability":false,"disableDryRun":false,"eksControlPlane":false,"enableZonalShift":false,"featureGates":{"capacityBuffer":false,"nodeOverlay":false,"nodeRepair":false,"reservedCapacity":true,"spotToSpotConsolidation":false,"staticCapacity":false},"ignoreDRARequests":true,"interruptionQueue":"","isolatedVPC":false,"kubeDNSServiceName":"kube-dns","kubeDNSServiceNamespace":"kube-system","minValuesPolicy":"Strict","preferencePolicy":"Respect","reservedENIs":"0","securityGroupRefreshInterval":"1m","subnetRefreshInterval":"1m","vmMemoryOverheadPercent":0.075}` | Global Settings to configure Karpenter |
...
| settings.clusterDNSIP | string | `""` | IP address of the cluster DNS service, used as the default kubelet clusterDNS for provisioned nodes. Takes precedence over the Service lookup below, and is itself overridden by clusterDNS on an EC2NodeClass. If empty, the address is read from the cluster DNS Service. |
...
| settings.kubeDNSServiceName | string | `"kube-dns"` | Name of the Service fronting cluster DNS. Karpenter reads its ClusterIP and uses it as the default kubelet clusterDNS for provisioned nodes. Set this if your cluster DNS Service is not named kube-dns. |
| settings.kubeDNSServiceNamespace | string | `"kube-system"` | Namespace of the Service fronting cluster DNS. |
