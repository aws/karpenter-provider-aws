...
* Behavior Changes:
  ...
  * Pods with the `karpenter.sh/do-not-disrupt` annotation now block node termination. Termination of a node with these pods will be blocked until those pods are removed, enter a terminating or terminal state, or the NodeClaims's [TerminationGracePeriod]({{<ref "../concepts/disruption.md#terminationgraceperiod">}}) has expired.
