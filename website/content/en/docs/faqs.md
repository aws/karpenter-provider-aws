...
Nodes are considered empty when they do not have any pods scheduled to them. Daemonsets pods and Failed pods are ignored. Karpenter will send a deletion request to the Kubernetes API, and graceful termination will be handled by termination finalizer.
<!-- todo talk about `ttlSecondsAfterUnderutilized -->
### When does Karpenter terminate expired nodes?
Nodes are considered expired when the current unix time exceeds their creation date plus `ttlSecondsUntilExpired`
