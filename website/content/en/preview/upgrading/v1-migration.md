...
* Karpenter runs a helm post-install-hook as part of upgrading to and from v1.0.0. If you're running Karpenter on a node not running on x86_64, you'll need to update your `values.postInstallHook.image` values in your helm `values.yaml` file to point to a compatible image.
