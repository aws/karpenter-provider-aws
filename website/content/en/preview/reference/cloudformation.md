...
provided that the request is made to a cluster with `RequestTag/kubernetes.io/cluster/${ClusterName}` set to owned, the `RequestTag/eks:eks-cluster-name` set to ${ClusterName}, and `RequestTag/topology.kubernetes.io/region` set to the current region.
...
The AllowScopedInstanceProfileTagActions Sid gives the Karpenter controller permission to tag an instance profile with [`iam:TagInstanceProfile`](https://docs.aws.amazon.com/IAM/latest/APIReference/API_TagInstanceProfile.html), provided that
`ResourceTag` `/kubernetes.io/cluster/${ClusterName}` is set to `owned` and
`topology.kubernetes.io/region` is set to the current region and `RequestTag` `kubernetes.io/cluster/${ClusterName} is set to "owned",
`eks:eks-cluster-name` is set to `${ClusterName}`, and `topology.kubernetes.io/region` is set to the current region.
