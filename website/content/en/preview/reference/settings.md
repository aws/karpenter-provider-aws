...
| NODE_KUBERNETES_VERSION | \-\-node-kubernetes-version | Pins the Kubernetes version used when provisioning nodes, instead of tracking the discovered control plane version. This is useful for node version downgrades, e.g. to support EKS control plane version upgrade rollbacks. The discovered control plane version continues to be used for Karpenter's own compatibility checks. Expected format: major.minor (e.g. 1.30).|
