...
| PRICING_ENDPOINT_REGION | \-\-pricing-endpoint-region | The AWS region containing the Price List Query API endpoint. If not specified, Karpenter selects an endpoint region based on the cluster region. This does not change the region for which prices are retrieved and has no effect when isolated-vpc is enabled.|
