...
* If your UserData sets `settings.autoscaling.should-wait`, Karpenter overrides it to `false`. Karpenter nodes are not part of an EC2 Auto Scaling group, so `should-wait = true` would prevent nodes from joining the cluster.
