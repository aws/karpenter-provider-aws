...
Note that [`amiFamily`]({{< ref "#specamifamily" >}}) determines the bootstrapping mode, while amiSelectorTerms specifies specific AMIs to be used. Therefore, you need to ensure consistency between [`amiFamily`]({{< ref "#specamifamily" >}}) and amiSelectorTerms to avoid conflicts during bootstrapping.
