...
Here are things you should know about NodeClaims:

* NodeClaims are **immutable** once created. You cannot modify a NodeClaim after it has been created.
* Each NodeClaim is **owned by a single NodePool** and references a single NodeClass.
* NodeClaims are **cluster-scoped** resources (not namespaced).
* Karpenter automatically manages the full lifecycle of NodeClaims. You should not need to create or delete them manually under normal operation.
* A NodeClaim maps one-to-one with a cloud provider instance and a Kubernetes Node.
* NodeClaims provide **observability** into Karpenter's provisioning decisions, including which instance type was selected, what zone it launched in, and the current lifecycle state.
...
The following is an annotated example of a NodeClaim resource. Keep in mind that NodeClaims are created and managed by Karpenter; you do not create them directly.
...
Each NodeClaim has an owner reference pointing to the NodePool that created it, with `blockOwnerDeletion: true`. This means deleting a NodePool will cascade-delete all of its NodeClaims.
