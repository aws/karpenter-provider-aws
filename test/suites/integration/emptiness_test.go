...
var _ = Describe("Emptiness", func() {
	...
	BeforeEach(func() {
		...
		nodePool.Spec.Disruption.ConsolidateAfter = &corev1.NillableDuration{Duration: lo.ToPtr(time.Duration(0))}
	...
	It("should terminate an empty node", func() {
		nodePool.Spec.Disruption.ConsolidateAfter = &corev1.NillableDuration{Duration: lo.ToPtr(time.Hour * 300)}
