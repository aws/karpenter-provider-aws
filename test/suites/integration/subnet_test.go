...
var _ = Describe("Subnets", func() {
	...
	It("should have the NodeClass status as not ready since subnets were not resolved", func() {
		...
		env.ExpectCreated(nodeClass)
		ExpectStatusConditions(env, env.Client, 1*time.Minute, nodeClass, status.Condition{Type: v1beta1.ConditionTypeSubnetsReady, Status: metav1.ConditionFalse, Message: "Unable to resolve Subnets"})
