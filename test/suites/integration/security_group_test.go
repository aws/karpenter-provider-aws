...
var _ = Describe("SecurityGroups", func() {
	It("should use the security-group-id selector", func() {
		...
		nodeClass.Spec.SecurityGroupSelectorTerms = lo.Map(securityGroups, func(sg aws.SecurityGroup, _ int) providerv1.SecurityGroupSelectorTerm {
			return providerv1.SecurityGroupSelectorTerm{
	...
	It("should update the EC2NodeClass status security groups", func() {
		...
		ExpectStatusConditions(env, env.Client, 1*time.Minute, nodeClass, status.Condition{Type: providerv1.ConditionTypeSecurityGroupsReady, Status: metav1.ConditionTrue})
	...
	It("should have the NodeClass status as not ready since security groups were not resolved", func() {
		...
		env.ExpectCreated(nodeClass)
		ExpectStatusConditions(env, env.Client, 1*time.Minute, nodeClass, status.Condition{Type: providerv1.ConditionTypeSecurityGroupsReady, Status: metav1.ConditionFalse, Message: "SecurityGroupSelector did not match any SecurityGroups"})
...
func EventuallyExpectSecurityGroups(env *aws.Environment, nodeClass *providerv1.EC2NodeClass) {
	securityGroups := env.GetSecurityGroups(map[string]string{"karpenter.sh/discovery": env.ClusterName})
	...
	Eventually(func(g Gomega) {
		...
		g.Expect(sets.New(lo.Map(temp.Status.SecurityGroups, func(s providerv1.SecurityGroup, _ int) string {
			return s.ID
		})...).Equal(ids))
