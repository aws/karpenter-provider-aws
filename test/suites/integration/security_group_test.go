...
var _ = Describe("SecurityGroups", func() {
	...
	It("should have the NodeClass status as not ready since security groups were not resolved", func() {
		...
		env.ExpectCreated(nodeClass)
		ExpectStatusConditions(env, env.Client, 1*time.Minute, nodeClass, status.Condition{Type: v1beta1.ConditionTypeSecurityGroupsReady, Status: metav1.ConditionFalse, Message: "Unable to resolve SecurityGroups"})
