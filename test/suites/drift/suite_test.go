...
var _ = Describe("Drift", func() {
	...
	Context("Budgets", func() {
		It("should respect budgets for empty drift", func() {
			...
			nodePool.Spec.Disruption.Budgets = []corev1.Budget{{
				Nodes: "50%",
			}}
			var numPods int32 = 6
			dep = coretest.Deployment(coretest.DeploymentOptions{
		...
		It("should respect budgets for non-empty delete drift", func() {
			...
			for _, pod := range pods {
				delete(pod.Annotations, corev1.DoNotDisruptAnnotationKey)
		...
		It("should respect budgets for non-empty replace drift", func() {
			...
			env.EventuallyExpectNotFound(lo.Map(originalNodeClaims, func(n *corev1.NodeClaim, _ int) client.Object { return n })...)
	...
	It("should disrupt nodes that have drifted due to AMIs", func() {
		...
		nodeClass.Spec.AMISelectorTerms = []providerv1.AMISelectorTerm{{ID: oldCustomAMI}}
		...
		nodeClass.Spec.AMISelectorTerms = []providerv1.AMISelectorTerm{{ID: amdAMI}}
	...
	It("should return drifted if the AMI no longer matches the existing NodeClaims instance type", func() {
		...
		nodeClass.Spec.AMIFamily = &providerv1.AMIFamilyAL2023
		nodeClass.Spec.AMISelectorTerms = []providerv1.AMISelectorTerm{{ID: armAMI}}
	...
	It("should disrupt nodes that have drifted due to securitygroup", func() {
		...
		sgTerms := []providerv1.SecurityGroupSelectorTerm{{ID: awssdk.StringValue(testSecurityGroup.GroupId)}}
		for _, id := range awsIDs {
			sgTerms = append(sgTerms, providerv1.SecurityGroupSelectorTerm{ID: id})
		}
		...
		sgTerms = lo.Reject(sgTerms, func(t providerv1.SecurityGroupSelectorTerm, _ int) bool {
	...
	It("should disrupt nodes that have drifted due to subnets", func() {
		...
		nodeClass.Spec.SubnetSelectorTerms = []providerv1.SubnetSelectorTerm{{ID: subnets[0].ID}}
		...
		nodeClass.Spec.SubnetSelectorTerms = []providerv1.SubnetSelectorTerm{{ID: subnets[1].ID}}
		env.ExpectCreatedOrUpdated(nodeClass)
	...
	DescribeTable("NodePool Drift", func(nodeClaimTemplate corev1.NodeClaimTemplate) {
		...
		Entry("Annotations", corev1.NodeClaimTemplate{
			ObjectMeta: corev1.ObjectMeta{
				Annotations: map[string]string{"keyAnnotationTest": "valueAnnotationTest"},
			},
		}),
		Entry("Labels", corev1.NodeClaimTemplate{
			ObjectMeta: corev1.ObjectMeta{
				Labels: map[string]string{"keyLabelTest": "valueLabelTest"},
			},
		}),
		Entry("Taints", corev1.NodeClaimTemplate{
			Spec: corev1.NodeClaimSpec{
				Taints: []v1.Taint{{Key: "example.com/another-taint-2", Effect: v1.TaintEffectPreferNoSchedule}},
			},
		}),
		Entry("Start-up Taints", corev1.NodeClaimTemplate{
			Spec: corev1.NodeClaimSpec{
				StartupTaints: []v1.Taint{{Key: "example.com/another-taint-2", Effect: v1.TaintEffectPreferNoSchedule}},
			},
		}),
		Entry("NodeRequirements", corev1.NodeClaimTemplate{
			Spec: corev1.NodeClaimSpec{
				Requirements: []corev1.NodeSelectorRequirementWithMinValues{{NodeSelectorRequirement: v1.NodeSelectorRequirement{Key: corev1.CapacityTypeLabelKey, Operator: v1.NodeSelectorOpIn, Values: []string{corev1.CapacityTypeSpot}}}},
			},
		}),
	)
	DescribeTable("EC2NodeClass", func(nodeClassSpec providerv1.EC2NodeClassSpec) {
		updatedNodeClass := test.EC2NodeClass(providerv1.EC2NodeClass{Spec: *nodeClass.Spec.DeepCopy()}, providerv1.EC2NodeClass{Spec: nodeClassSpec})
		...
		Entry("UserData", providerv1.EC2NodeClassSpec{UserData: awssdk.String("#!/bin/bash\necho \"Hello, AL2023\"")}),
		Entry("Tags", providerv1.EC2NodeClassSpec{Tags: map[string]string{"keyTag-test-3": "valueTag-test-3"}}),
		Entry("MetadataOptions", providerv1.EC2NodeClassSpec{MetadataOptions: &providerv1.MetadataOptions{HTTPTokens: awssdk.String("required"), HTTPPutResponseHopLimit: awssdk.Int64(10)}}),
		Entry("BlockDeviceMappings", providerv1.EC2NodeClassSpec{BlockDeviceMappings: []*providerv1.BlockDeviceMapping{
			{
		...
		Entry("DetailedMonitoring", providerv1.EC2NodeClassSpec{DetailedMonitoring: awssdk.Bool(true)}),
		Entry("AMIFamily", providerv1.EC2NodeClassSpec{AMIFamily: awssdk.String(providerv1.AMIFamilyBottlerocket)}),
	)
	...
	It("should drift the EC2NodeClass on BlockDeviceMappings volume size update", func() {
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(nodeClaim.StatusConditions().Get(corev1.ConditionTypeDrifted)).ToNot(BeNil())
			g.Expect(nodeClaim.StatusConditions().Get(corev1.ConditionTypeDrifted).IsTrue()).To(BeTrue())
		}).Should(Succeed())
	...
	It("should update the nodepool-hash annotation on the nodepool and nodeclaim when the nodepool's nodepool-hash-version annotation does not match the controller hash version", func() {
		...
		env.EventuallyExpectHealthyPodCount(selector, numPods)
		nodeClaim := env.EventuallyExpectCreatedNodeClaimCount("==", 1)[0]
		nodePool = env.ExpectExists(nodePool).(*corev1.NodePool)
		expectedHash := nodePool.Hash()

		By(fmt.Sprintf("expect nodepool %s and nodeclaim %s to contain %s and %s annotations", nodePool.Name, nodeClaim.Name, corev1.NodePoolHashAnnotationKey, corev1.NodePoolHashVersionAnnotationKey))
		Eventually(func(g Gomega) {
			...
			g.Expect(nodePool.Annotations).To(HaveKeyWithValue(corev1.NodePoolHashAnnotationKey, expectedHash))
			g.Expect(nodePool.Annotations).To(HaveKeyWithValue(corev1.NodePoolHashVersionAnnotationKey, corev1.NodePoolHashVersion))
			g.Expect(nodeClaim.Annotations).To(HaveKeyWithValue(corev1.NodePoolHashAnnotationKey, expectedHash))
			g.Expect(nodeClaim.Annotations).To(HaveKeyWithValue(corev1.NodePoolHashVersionAnnotationKey, corev1.NodePoolHashVersion))
		}).WithTimeout(30 * time.Second).Should(Succeed())

		nodePool.Annotations = lo.Assign(nodePool.Annotations, map[string]string{
			corev1.NodePoolHashAnnotationKey:        "test-hash-1",
			corev1.NodePoolHashVersionAnnotationKey: "test-hash-version-1",
		})
		...
		nodeClaim.Annotations = lo.Assign(nodePool.Annotations, map[string]string{
			corev1.NodePoolHashAnnotationKey:        "test-hash-2",
			corev1.NodePoolHashVersionAnnotationKey: "test-hash-version-2",
		})
	...
	It("should update the ec2nodeclass-hash annotation on the ec2nodeclass and nodeclaim when the ec2nodeclass's ec2nodeclass-hash-version annotation does not match the controller hash version", func() {
		...
		env.EventuallyExpectHealthyPodCount(selector, numPods)
		nodeClaim := env.EventuallyExpectCreatedNodeClaimCount("==", 1)[0]
		nodeClass = env.ExpectExists(nodeClass).(*providerv1.EC2NodeClass)
		expectedHash := nodeClass.Hash()

		By(fmt.Sprintf("expect nodeclass %s and nodeclaim %s to contain %s and %s annotations", nodeClass.Name, nodeClaim.Name, providerv1.AnnotationEC2NodeClassHash, providerv1.AnnotationEC2NodeClassHashVersion))
		Eventually(func(g Gomega) {
			g.Expect(env.Client.Get(env.Context, client.ObjectKeyFromObject(nodeClass), nodeClass)).To(Succeed())
			g.Expect(env.Client.Get(env.Context, client.ObjectKeyFromObject(nodeClaim), nodeClaim)).To(Succeed())

			g.Expect(nodeClass.Annotations).To(HaveKeyWithValue(providerv1.AnnotationEC2NodeClassHash, expectedHash))
			g.Expect(nodeClass.Annotations).To(HaveKeyWithValue(providerv1.AnnotationEC2NodeClassHashVersion, providerv1.EC2NodeClassHashVersion))
			g.Expect(nodeClaim.Annotations).To(HaveKeyWithValue(providerv1.AnnotationEC2NodeClassHash, expectedHash))
			g.Expect(nodeClaim.Annotations).To(HaveKeyWithValue(providerv1.AnnotationEC2NodeClassHashVersion, providerv1.EC2NodeClassHashVersion))
		}).WithTimeout(30 * time.Second).Should(Succeed())

		nodeClass.Annotations = lo.Assign(nodeClass.Annotations, map[string]string{
			providerv1.AnnotationEC2NodeClassHash:        "test-hash-1",
		...
		nodeClaim.Annotations = lo.Assign(nodePool.Annotations, map[string]string{
			providerv1.AnnotationEC2NodeClassHash:        "test-hash-2",
	...
	Context("Failure", func() {
		It("should not disrupt a drifted node if the replacement node never registers", func() {
			...
			nodeClass.Spec.AMISelectorTerms = []providerv1.AMISelectorTerm{{ID: env.GetAMIBySSMPath("/aws/service/ami-amazon-linux-latest/amzn2-ami-hvm-x86_64-ebs")}}
			env.ExpectCreatedOrUpdated(nodeClass)
			...
			Consistently(func(g Gomega) {
				...
				startingNodeClaimUIDs := sets.New(lo.Map(startingNodeClaimState, func(nc *corev1.NodeClaim, _ int) types.UID { return nc.UID })...)
				nodeClaimUIDs := sets.New(lo.Map(nodeClaims.Items, func(nc corev1.NodeClaim, _ int) types.UID { return nc.UID })...)
		...
		It("should not disrupt a drifted node if the replacement node registers but never initialized", func() {
			...
			Consistently(func(g Gomega) {
				nodeClaims := &corev1.NodeClaimList{}
				g.Expect(env.Client.List(env, nodeClaims, client.HasLabels{coretest.DiscoveryLabel})).To(Succeed())
				startingNodeClaimUIDs := sets.New(lo.Map(startingNodeClaimState, func(m *corev1.NodeClaim, _ int) types.UID { return m.UID })...)
				nodeClaimUIDs := sets.New(lo.Map(nodeClaims.Items, func(m corev1.NodeClaim, _ int) types.UID { return m.UID })...)
				g.Expect(nodeClaimUIDs.IsSuperset(startingNodeClaimUIDs)).To(BeTrue())
			}, "2m").Should(Succeed())
		})
