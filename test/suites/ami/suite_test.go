...
var _ = Describe("AMI", func() {
	...
	Context("AMIFamily", func() {
		...
		It("should have ec2nodeClass status as not ready since AMI was not resolved", func() {
			...
			env.ExpectCreated(nodeClass)
			ExpectStatusConditions(env, env.Client, 1*time.Minute, nodeClass, status.Condition{Type: v1beta1.ConditionTypeAMIsReady, Status: metav1.ConditionFalse, Message: "Unable to resolve AMI"})
