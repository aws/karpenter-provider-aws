...
upstream-e2etests: tidy download
	...
	CLUSTER_NAME=${CLUSTER_NAME} envsubst < $(shell pwd)/test/pkg/environment/aws/invalid_ec2nodeclass.yaml > ${INVALIDFILE}
	go test \
		-count 1 \
		-timeout 3.25h \
		-v \
