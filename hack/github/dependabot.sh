...
for DIR in "${DIRS[@]}"; do
  i=$i dir=$DIR go tool -modfile=go.tools.mod yq -i '.updates[env(i)] = {"package-ecosystem": "github-actions", "directory": env(dir), "schedule": {"interval": "weekly"}, "groups": {"action-deps": {"patterns": ["*"]}}}' .github/dependabot.yaml
  i=$((i+1))
done