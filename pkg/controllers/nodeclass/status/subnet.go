...
func (s *Subnet) Reconcile(ctx context.Context, nodeClass *providerv1.EC2NodeClass) (reconcile.Result, error) {
	...
	if len(subnets) == 0 {
		...
		nodeClass.StatusConditions().SetFalse(providerv1.ConditionTypeSubnetsReady, "SubnetsNotFound", "SubnetSelector did not match any Subnets")
	...
	nodeClass.StatusConditions().SetTrue(providerv1.ConditionTypeSubnetsReady)
