...
func (c *Controller) handleMessage(ctx context.Context, nodeClaimInstanceIDMap map[string]*providerv1.NodeClaim,
...
func (c *Controller) handleNodeClaim(ctx context.Context, msg messages.Message, nodeClaim *providerv1.NodeClaim, node *v1.Node) error {
	...
	actionsPerformed.With(
		prometheus.Labels{
			...
			metrics.NodePoolLabel: nodeClaim.Labels[providerv1.NodePoolLabelKey],
		},
	).Inc()
	...
	if msg.Kind() == messages.SpotInterruptionKind {
		...
		if zone != "" && instanceType != "" {
			c.unavailableOfferingsCache.MarkUnavailable(ctx, string(msg.Kind()), instanceType, zone, providerv1.CapacityTypeSpot)
		}
	}
	if action != NoAction {
...
func (c *Controller) deleteNodeClaim(ctx context.Context, nodeClaim *providerv1.NodeClaim, node *v1.Node) error {
	...
	metrics.NodeClaimsTerminatedCounter.With(prometheus.Labels{
		...
		metrics.NodePoolLabel:     nodeClaim.Labels[providerv1.NodePoolLabelKey],
		metrics.CapacityTypeLabel: nodeClaim.Labels[providerv1.CapacityTypeLabelKey],
	}).Inc()
	return nil
}

// notifyForMessage publishes the relevant alert based on the message kind
func (c *Controller) notifyForMessage(msg messages.Message, nodeClaim *providerv1.NodeClaim, n *v1.Node) {
	switch msg.Kind() {
...
func (c *Controller) makeNodeClaimInstanceIDMap(ctx context.Context) (map[string]*providerv1.NodeClaim, error) {
	m := map[string]*providerv1.NodeClaim{}
