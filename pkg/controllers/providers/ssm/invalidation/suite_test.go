...
import (
	...
	"github.com/aws/karpenter/pkg/controllers/providers/ssm/invalidation"
...
var _ = Describe("SSM Invalidation Controller", func() {
	...
	It("shouldn't invalidate cache entries for non-deprecated AMIs", func() {
		_, err := awsEnv.AMIProvider.Get(ctx, nodeClass, nil)
		Expect(err).To(BeNil())
		currentEntries := getSSMCacheEntries()
		Expect(len(currentEntries)).To(Equal(3))
		...
		Expect(len(updatedEntries)).To(Equal(3))
