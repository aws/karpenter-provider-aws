...
func benchmarkNotificationController(b *testing.B, messageCount int) {
	...
	notificationController := interruption.NewController(env.Client, fakeClock, recorder, cluster, providers.sqsProvider, unavailableOfferingsCache)
...
func (p *providerSet) makeInfrastructure(ctx context.Context) error {
	infraProvider := nodetemplate.NewProvider(p.sqsProvider, p.eventBridgeProvider)
