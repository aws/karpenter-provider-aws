...
func (p *SecurityGroupProvider) getSecurityGroups(ctx context.Context, filters []*ec2.Filter) ([]*ec2.SecurityGroup, error) {
	...
	if p.cm.HasChanged("security-groups", output.SecurityGroups) {
		ctx = logging.WithLogger(ctx, logging.FromContext(ctx).With("security-group", p.securityGroupIds(output.SecurityGroups)))
		logging.FromContext(ctx).Debugf("Discovered security groups")
	}
