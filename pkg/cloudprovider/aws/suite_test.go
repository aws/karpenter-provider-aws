...
var _ = Describe("Allocation", func() {
	...
	Context("Reconciliation", func() {
		...
		Context("Insufficient Capacity Error Cache", func() {
			...
			It("should launch smaller instances than optimal if larger instance launch results in Insufficient Capacity Error", func() {
				...
				twoInstanceProvisioner.Spec.Constraints.Requirements = append(twoInstanceProvisioner.Spec.Constraints.Requirements,
	...
	Context("Defaulting", func() {
		...
		It("should default requirements", func() {
			provisioner.SetDefaults(ctx)
			sr := scheduling.NewRequirements(provisioner.Spec.Requirements...)
			Expect(sr.CapacityTypes().UnsortedList()).To(ConsistOf(v1alpha1.CapacityTypeOnDemand))
			Expect(sr.Architectures().UnsortedList()).To(ConsistOf(v1alpha5.ArchitectureAmd64))
		})
	})
