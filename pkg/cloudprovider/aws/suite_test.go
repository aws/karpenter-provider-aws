	...
	Context("Reconciliation", func() {
		...
		Context("Metadata Options", func() {
			It("should default metadata options on generated launch template", func() {
				...
				Expect(fakeEC2API.CalledWithCreateLaunchTemplateInput.Cardinality()).To(Equal(1))
				input := fakeEC2API.CalledWithCreateLaunchTemplateInput.Pop().(*ec2.CreateLaunchTemplateInput)
				Expect(*input.LaunchTemplateData.MetadataOptions.HttpEndpoint).To(Equal(defaultMetadataOptionsHTTPEndpoint))
				Expect(*input.LaunchTemplateData.MetadataOptions.HttpProtocolIpv6).To(Equal(defaultMetadataOptionsHTTPProtocolIpv6))
				Expect(*input.LaunchTemplateData.MetadataOptions.HttpPutResponseHopLimit).To(Equal(int64(defaultMetadataOptionsHTTPPutResponseHopLimit)))
				Expect(*input.LaunchTemplateData.MetadataOptions.HttpTokens).To(Equal(defaultMetadataOptionsHTTPTokens))
			})
