...
func (c *CloudProvider) isNodeClassDrifted(ctx context.Context, nodeClaim *corev1.NodeClaim, nodePool *corev1.NodePool, nodeClass *providerv1.EC2NodeClass) (cloudprovider.DriftReason, error) {
...
func (c *CloudProvider) isAMIDrifted(ctx context.Context, nodeClaim *corev1.NodeClaim, nodePool *corev1.NodePool,
	instance *instance.Instance, nodeClass *providerv1.EC2NodeClass) (cloudprovider.DriftReason, error) {
...
func (c *CloudProvider) isSubnetDrifted(instance *instance.Instance, nodeClass *providerv1.EC2NodeClass) (cloudprovider.DriftReason, error) {
	// subnets need to be found to check for drift
	if len(nodeClass.Status.Subnets) == 0 {
		return "", fmt.Errorf("no subnets are discovered")
	}

	_, found := lo.Find(nodeClass.Status.Subnets, func(subnet providerv1.Subnet) bool {
...
func (c *CloudProvider) areSecurityGroupsDrifted(ec2Instance *instance.Instance, nodeClass *providerv1.EC2NodeClass) (cloudprovider.DriftReason, error) {
	securityGroupIds := sets.New(lo.Map(nodeClass.Status.SecurityGroups, func(sg providerv1.SecurityGroup, _ int) string { return sg.ID })...)
...
func (c *CloudProvider) areStaticFieldsDrifted(nodeClaim *corev1.NodeClaim, nodeClass *providerv1.EC2NodeClass) cloudprovider.DriftReason {
	nodeClassHash, foundNodeClassHash := nodeClass.Annotations[providerv1.AnnotationEC2NodeClassHash]
	nodeClassHashVersion, foundNodeClassHashVersion := nodeClass.Annotations[providerv1.AnnotationEC2NodeClassHashVersion]
	nodeClaimHash, foundNodeClaimHash := nodeClaim.Annotations[providerv1.AnnotationEC2NodeClassHash]
