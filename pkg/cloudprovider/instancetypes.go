...
func (p *InstanceTypeProvider) getInstanceTypeZones(ctx context.Context, provider *v1alpha1.AWS) (map[string]sets.String, error) {
	...
	if p.cm.HasChanged("zonal-offerings", provider.SubnetSelector) {
		ctx = logging.WithLogger(ctx, logging.FromContext(ctx).With("subnetSelector", pretty.Concise(provider.SubnetSelector)))
		logging.FromContext(ctx).Debugf("Discovered EC2 instance types zonal offerings for subnets")
	}
...
func (p *InstanceTypeProvider) getInstanceTypes(ctx context.Context) (map[string]*ec2.InstanceTypeInfo, error) {
	...
	if p.cm.HasChanged("instance-types", instanceTypes) {
		ctx = logging.WithLogger(ctx, logging.FromContext(ctx).With("numberOfInstanceTypes", len(instanceTypes)))
		logging.FromContext(ctx).Debugf("Discovered EC2 instance types")
	}
