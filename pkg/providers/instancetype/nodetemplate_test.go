...
var _ = Describe("NodeTemplate/InstanceTypes", func() {
	var nodeTemplate, windowsNodeTemplate *v1alpha1.AWSNodeTemplate
	var provisioner, windowsProvisioner *v1alpha5.Provisioner
	BeforeEach(func() {
	...
	It("should order the instance types by price and only consider the spot types that are cheaper than the cheapest on-demand", func() {
		...
		ExpectApplied(ctx, env.Client, provisioner, nodeTemplate)
		awsEnv.EC2API.DescribeSpotPriceHistoryOutput.Set(generateSpotPricing(cloudProvider, nodepoolutil.New(provisioner)))
	...
	It("should launch instances for AWS NeuronCore resource requests", func() {
		...
		pods := []*v1.Pod{
			...
			coretest.UnschedulablePod(coretest.PodOptions{
				ResourceRequirements: v1.ResourceRequirements{
					Requests: v1.ResourceList{v1alpha1.ResourceAWSNeuronCore: resource.MustParse("32")},
					Limits:   v1.ResourceList{v1alpha1.ResourceAWSNeuronCore: resource.MustParse("32")},
				},
			}),
			// Should pack onto a separate instance
			coretest.UnschedulablePod(coretest.PodOptions{
				ResourceRequirements: v1.ResourceRequirements{
					Requests: v1.ResourceList{v1alpha1.ResourceAWSNeuronCore: resource.MustParse("64")},
					Limits:   v1.ResourceList{v1alpha1.ResourceAWSNeuronCore: resource.MustParse("64")},
				},
			}),
		}
		ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, prov, pods...)
	...
	It("should launch instances for AWS Neuron resource requests", func() {
		...
		pods := []*v1.Pod{
			...
			coretest.UnschedulablePod(coretest.PodOptions{
				ResourceRequirements: v1.ResourceRequirements{
					Requests: v1.ResourceList{v1alpha1.ResourceAWSNeuron: resource.MustParse("8")},
					Limits:   v1.ResourceList{v1alpha1.ResourceAWSNeuron: resource.MustParse("8")},
				},
			}),
			// Should pack onto a separate instance
			coretest.UnschedulablePod(coretest.PodOptions{
				ResourceRequirements: v1.ResourceRequirements{
					Requests: v1.ResourceList{v1alpha1.ResourceAWSNeuron: resource.MustParse("16")},
					Limits:   v1.ResourceList{v1alpha1.ResourceAWSNeuron: resource.MustParse("16")},
				},
			}),
		}
		ExpectProvisioned(ctx, env.Client, cluster, cloudProvider, prov, pods...)
	...
	Context("Insufficient Capacity Error Cache", func() {
		It("should launch instances of different type on second reconciliation attempt with Insufficient Capacity Error Cache fallback", func() {
			awsEnv.EC2API.InsufficientCapacityPools.Set([]fake.CapacityPool{{CapacityType: v1alpha5.CapacityTypeOnDemand, InstanceType: "inf1.24xlarge", Zone: "test-zone-1a"}})
			ExpectApplied(ctx, env.Client, provisioner, nodeTemplate)
			pods := []*v1.Pod{
				coretest.UnschedulablePod(coretest.PodOptions{
