...
func computeRequirements(
	...
	if len(capacityReservations) != 0 {
		requirements.Add(scheduling.NewRequirement(v1.LabelCapacityReservationID, corev1.NodeSelectorOpIn, lo.Map(capacityReservations, func(cr v1.CapacityReservation, _ int) string {
			return cr.ID
		})...))
	} else {
		requirements.Add(scheduling.NewRequirement(v1.LabelCapacityReservationID, corev1.NodeSelectorOpDoesNotExist))
	}
