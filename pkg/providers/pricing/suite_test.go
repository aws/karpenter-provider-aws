...
var _ = Describe("Pricing", func() {
	...
	It("should not return spot prices more expensive than on demand price", func() {
		now := time.Now()
		awsEnv.EC2API.DescribeSpotPriceHistoryOutput.Set(&ec2.DescribeSpotPriceHistoryOutput{
			SpotPriceHistory: []*ec2.SpotPrice{
				{
		...
		Expect(getPricingEstimateMetricValue("c99.large", ec2.UsageClassTypeSpot, "test-zone-1a")).To(BeNumerically("==", 1.13))

		price, ok = awsEnv.PricingProvider.SpotPrice("c99.large", "test-zone-1b")
		Expect(ok).To(BeFalse())
		Expect(price).To(BeNumerically("==", 0.0))
		Expect(getPricingEstimateMetricValue("c99.large", ec2.UsageClassTypeSpot, "test-zone-1b")).To(BeNumerically("==", 1.50))
	})
