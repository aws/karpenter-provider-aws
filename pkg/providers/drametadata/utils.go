...
// Attributes converts scraped device attributes into their DRA form. The scraped names are already
// the fully qualified names the driver publishes, so they pass through unchanged, and each value
// keeps its type: a version has to stay a version for a semver CEL selector to evaluate against it.
...
// Capacity converts scraped device capacities into their DRA form. It panics on a malformed
// quantity, failing fast at startup on a bad generated entry rather than at scheduling time.
