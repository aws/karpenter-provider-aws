...
type Provider interface {
	EnsureAll(context.Context, *providerv1.EC2NodeClass, *corev1.NodeClaim,
		[]*cloudprovider.InstanceType, string, map[string]string) ([]*LaunchTemplate, error)
	DeleteAll(context.Context, *providerv1.EC2NodeClass) error
...
func (p *DefaultProvider) EnsureAll(ctx context.Context, nodeClass *providerv1.EC2NodeClass, nodeClaim *corev1.NodeClaim,
	instanceTypes []*cloudprovider.InstanceType, capacityType string, tags map[string]string) ([]*LaunchTemplate, error) {

	p.Lock()
	defer p.Unlock()

	options, err := p.createAMIOptions(ctx, nodeClass, lo.Assign(nodeClaim.Labels, map[string]string{corev1.CapacityTypeLabelKey: capacityType}), tags)
	if err != nil {
		return nil, err
	}
...
func (p *DefaultProvider) createAMIOptions(ctx context.Context, nodeClass *providerv1.EC2NodeClass, labels, tags map[string]string) (*amifamily.Options, error) {
	...
	for k := range labels {
		labelDomain := corev1.GetLabelDomain(k)
...
func (p *DefaultProvider) createLaunchTemplate(ctx context.Context, options *amifamily.LaunchTemplate) (*ec2.LaunchTemplate, error) {
	...
	if options.CapacityType == corev1.CapacityTypeSpot {
	...
	output, err := p.ec2api.CreateLaunchTemplateWithContext(ctx, &ec2.CreateLaunchTemplateInput{
		...
		LaunchTemplateData: &ec2.RequestLaunchTemplateData{
			...
			SecurityGroupIds: lo.Ternary(networkInterfaces != nil, nil, lo.Map(options.SecurityGroups, func(s providerv1.SecurityGroup, _ int) *string { return aws.String(s.ID) })),
		...
		TagSpecifications: []*ec2.TagSpecification{
			{
				...
				Tags:         utils.MergeTags(options.Tags, map[string]string{providerv1.TagManagedLaunchTemplate: options.ClusterName, providerv1.LabelNodeClass: options.NodeClassName}),
			},
		},
	})
	if err != nil {
	...
	if options.EFACount != 0 {
		return lo.Times(options.EFACount, func(i int) *ec2.LaunchTemplateInstanceNetworkInterfaceSpecificationRequest {
			return &ec2.LaunchTemplateInstanceNetworkInterfaceSpecificationRequest{
				...
				Groups:        lo.Map(options.SecurityGroups, func(s providerv1.SecurityGroup, _ int) *string { return aws.String(s.ID) }),
...
func (p *DefaultProvider) blockDeviceMappings(blockDeviceMappings []*providerv1.BlockDeviceMapping) []*ec2.LaunchTemplateBlockDeviceMappingRequest {
...
func (p *DefaultProvider) hydrateCache(ctx context.Context) {
	clusterName := options.FromContext(ctx).ClusterName
	ctx = log.IntoContext(ctx, log.FromContext(ctx).WithValues("tag-key", providerv1.TagManagedLaunchTemplate, "tag-value", clusterName))
	if err := p.ec2api.DescribeLaunchTemplatesPagesWithContext(ctx, &ec2.DescribeLaunchTemplatesInput{
		Filters: []*ec2.Filter{{Name: aws.String(fmt.Sprintf("tag:%s", providerv1.TagManagedLaunchTemplate)), Values: []*string{aws.String(clusterName)}}},
...
func (p *DefaultProvider) getInstanceProfile(nodeClass *providerv1.EC2NodeClass) (string, error) {
...
func (p *DefaultProvider) DeleteAll(ctx context.Context, nodeClass *providerv1.EC2NodeClass) error {
	clusterName := options.FromContext(ctx).ClusterName
	var ltNames []*string
	if err := p.ec2api.DescribeLaunchTemplatesPagesWithContext(ctx, &ec2.DescribeLaunchTemplatesInput{
		Filters: []*ec2.Filter{
			{Name: aws.String(fmt.Sprintf("tag:%s", providerv1.TagManagedLaunchTemplate)), Values: []*string{aws.String(clusterName)}},
			{Name: aws.String(fmt.Sprintf("tag:%s", providerv1.LabelNodeClass)), Values: []*string{aws.String(nodeClass.Name)}},
		},
	}, func(output *ec2.DescribeLaunchTemplatesOutput, _ bool) bool {
		for _, lt := range output.LaunchTemplates {
