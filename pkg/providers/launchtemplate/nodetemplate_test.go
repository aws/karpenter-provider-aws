...
var _ = Describe("NodeTemplate/LaunchTemplates", func() {
	...
	Context("User Data", func() {
		...
		Context("Bottlerocket", func() {
			It("should merge in custom user data", func() {
				...
				Expect(err).To(BeNil())
				nodeTemplate.Spec.UserData = aws.String(fmt.Sprintf(string(content), v1alpha5.ProvisionerNameLabelKey))
				nodeTemplate.Spec.AMIFamily = &v1alpha1.AMIFamilyBottlerocket
				...
				Expect(err).To(BeNil())
				ExpectLaunchTemplatesCreatedWithUserData(fmt.Sprintf(string(content), v1alpha5.ProvisionerNameLabelKey, provisioner.Name))
			})
		...
		Context("AL2 Custom UserData", func() {
			It("should merge in custom user data", func() {
				...
				Expect(err).To(BeNil())
				expectedUserData := fmt.Sprintf(string(content), v1alpha5.ProvisionerNameLabelKey, newProvisioner.Name)
