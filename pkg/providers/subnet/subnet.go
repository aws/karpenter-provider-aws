...
type Provider interface {
	...
	List(context.Context, *providerv1.EC2NodeClass) ([]*ec2.Subnet, error)
	ZonalSubnetsForLaunch(context.Context, *providerv1.EC2NodeClass, []*cloudprovider.InstanceType, string) (map[string]*Subnet, error)
...
func (p *DefaultProvider) List(ctx context.Context, nodeClass *providerv1.EC2NodeClass) ([]*ec2.Subnet, error) {
	p.Lock()
	defer p.Unlock()
	...
	if p.cm.HasChanged(fmt.Sprintf("subnets/%s", nodeClass.Name), lo.Keys(subnets)) {
		log.FromContext(ctx).
			WithValues("subnets", lo.Map(lo.Values(subnets), func(s *ec2.Subnet, _ int) providerv1.Subnet {
...
func (p *DefaultProvider) ZonalSubnetsForLaunch(ctx context.Context, nodeClass *providerv1.EC2NodeClass, instanceTypes []*cloudprovider.InstanceType, capacityType string) (map[string]*Subnet, error) {
...
func getFilterSets(terms []providerv1.SubnetSelectorTerm) (res [][]*ec2.Filter) {
