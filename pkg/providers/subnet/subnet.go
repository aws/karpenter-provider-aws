...
func (p *DefaultProvider) CheckAnyPublicIPAssociations(ctx context.Context, nodeClass *v1beta1.EC2NodeClass) (bool, error) {
	if len(nodeClass.Status.Subnets) == 0 {
		return false, fmt.Errorf("no subnets matched selector %v", nodeClass.Spec.SubnetSelectorTerms)
	}

	filterSets := getFilterSets(nodeClass.Spec.SubnetSelectorTerms)
	if len(filterSets) == 0 {
		return false, fmt.Errorf("no cached subnets matched selector %v", nodeClass.Spec.SubnetSelectorTerms)
	}
	hash := lo.Must1(hashstructure.Hash(filterSets, hashstructure.FormatV2, &hashstructure.HashOptions{SlicesAsSets: true}))
	cachedSubnets, ok := p.cache.Get(fmt.Sprint(hash))
	if !ok {
		return false, fmt.Errorf("no cached subnets matched selector %v", nodeClass.Spec.SubnetSelectorTerms)
	}

	subnets := cachedSubnets.([]*ec2.Subnet)
...
func (p *DefaultProvider) ZonalSubnetsForLaunch(ctx context.Context, nodeClass *v1beta1.EC2NodeClass, instanceTypes []*cloudprovider.InstanceType, capacityType string) (map[string]*ec2.Subnet, error) {
	...
	filterSets := getFilterSets(nodeClass.Spec.SubnetSelectorTerms)
	if len(filterSets) == 0 {
		return zonalSubnets, fmt.Errorf("no cached subnets matched selector %v", nodeClass.Spec.SubnetSelectorTerms)
	}
	hash := lo.Must1(hashstructure.Hash(filterSets, hashstructure.FormatV2, &hashstructure.HashOptions{SlicesAsSets: true}))
	cachedSubnets, ok := p.cache.Get(fmt.Sprint(hash))
	if !ok {
		for _, subnet := range nodeClass.Status.Subnets {
			zonalSubnets[subnet.Zone] = &ec2.Subnet{
