...
// dynamicResources holds the GPU DRA metadata keyed by instance type name, built once at package
// init and immutable afterwards, making it safe to share by pointer.
var dynamicResources = buildDynamicResources()

// buildDynamicResources converts the scraped GPU metadata into one template per instance type: one
	...
	for instanceType, metadata := range drametadata.GPUMetadataByInstanceType {
		...
		devices := lo.Map(metadata.Devices, func(device drametadata.DRADevice, index int) cloudprovider.Device {
			return cloudprovider.Device{
				...
				Capacity:   drametadata.Capacity(device.Capacity),
			}
		})
