...
func (p *DefaultProvider) getDefaultAMIs(ctx context.Context, nodeClass *v1.EC2NodeClass) (res AMIs, err error) {
...
func (p *DefaultProvider) getAMIs(ctx context.Context, terms []v1.AMISelectorTerm) (AMIs, error) {
	...
	for _, filtersAndOwners := range filterAndOwnerSets {
		if err = p.ec2api.DescribeImagesPagesWithContext(ctx, &ec2.DescribeImagesInput{
			...
			for i := range page.Images {
				...
				if !v1.WellKnownArchitectures.Has(reqs.Get(corev1.LabelArchStable).Any()) {
					continue
				}
...
func GetFilterAndOwnerSets(terms []v1.AMISelectorTerm) (res []FiltersAndOwners) {
...
func (p *DefaultProvider) getRequirementsFromImage(ec2Image *ec2.Image) scheduling.Requirements {
	...
	if value, ok := v1.AWSToKubeArchitectures[architecture]; ok {
		architecture = value
	}
	requirements.Add(scheduling.NewRequirement(corev1.LabelArchStable, corev1.NodeSelectorOpIn, architecture))
	return requirements
}
