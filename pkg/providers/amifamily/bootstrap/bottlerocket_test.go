...
func TestBottlerocket(t *testing.T) {
	...
	RunSpecs(t, "Bottlerocket Bootstrap")
}

var _ = Describe("Bottlerocket Configuration", func() {
	var br bootstrap.Bottlerocket

	BeforeEach(func() {
		br = bootstrap.Bottlerocket{
...
cluster-dns-ip = "8.8.8.8"`
			...
			config, err := bootstrap.NewBottlerocketConfig(&customData)
			Expect(err).ToNot(HaveOccurred())
...
cluster-dns-ip = ["8.8.8.8", "8.8.4.4"]`
		...
		It("should handle empty custom user data", func() {
			config, err := bootstrap.NewBottlerocketConfig(nil)
			Expect(err).ToNot(HaveOccurred())
...
cluster-dns-ip = invalid-value`
			...
			_, err := bootstrap.NewBottlerocketConfig(&customData)
