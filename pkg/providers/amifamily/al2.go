...
func (a AL2) DefaultAMIs(version string) []DefaultAMIOutput {
	return []DefaultAMIOutput{
		...
		{
			Query: fmt.Sprintf("/aws/service/eks/optimized-ami/%s/amazon-linux-2-%s/recommended/image_id", version, corev1.ArchitectureArm64),
			Requirements: scheduling.NewRequirements(
...
func (a AL2) UserData(kubeletConfig *providerv1.KubeletConfiguration, taints []v1.Taint, labels map[string]string, caBundle *string, _ []*cloudprovider.InstanceType, customUserData *string, instanceStorePolicy *providerv1.InstanceStorePolicy) bootstrap.Bootstrapper {
...
func (a AL2) DefaultBlockDeviceMappings() []*providerv1.BlockDeviceMapping {
