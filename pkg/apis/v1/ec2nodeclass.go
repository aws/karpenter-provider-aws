...
type EC2NodeClassSpec struct {
	...
	// +kubebuilder:validation:Enum:={AL2,AL2023,Bottlerocket,Bottlerocket-FIPS,Custom,Windows2019,Windows2022}
	// +optional
...
type AMISelectorTerm struct {
	...
	// Valid families include: al2, al2023, bottlerocket, bottlerocket-fips, windows2019, and windows2022.
...
type EC2NodeClass struct {
	...
	// +kubebuilder:validation:XValidation:message="if set, amiFamily must be 'Bottlerocket' or 'Bottlerocket-FIPS' or 'Custom' when using a Bottlerocket alias",rule="!has(self.amiFamily) || (self.amiSelectorTerms.exists(x, has(x.alias) && (x.alias.find('^[^@]+') == 'bottlerocket' || x.alias.find('^[^@]+') == 'bottlerocket-fips')) ? (self.amiFamily == 'Custom' || self.amiFamily == 'Bottlerocket' || self.amiFamily == 'Bottlerocket-FIPS') : true)"
